...
function neutron_xyz_install {
    setup_develop $NEUTRON_VPNAAS_DIR
    echo "This is the dir",$NEUTRON_VPNAAS_DIR
...
function neutron_xyz_configure_common {
    cp $NEUTRON_VPNAAS_DIR/etc/neutron_xyz.conf.sample $NEUTRON_VPNAAS_CONF
...
function neutron_xyz_configure_db {
    $NEUTRON_BIN_DIR/neutron-db-manage --subproject neutron-xyz --config-file $NEUTRON_CONF --config-file /$Q_PLUGIN_CONF_FILE upgrade head
}

function neutron_xyz_configure_agent {
    ...
    if [[ "$IPSEC_PACKAGE" == "strongswan" ]]; then
        if is_fedora; then
            iniset_multiline $conf_file vpnagent vpn_device_driver neutron_xyz.services.vpn.device_drivers.fedora_strongswan_ipsec.FedoraStrongSwanDriver
        else
            iniset_multiline $conf_file vpnagent vpn_device_driver neutron_xyz.services.vpn.device_drivers.strongswan_ipsec.StrongSwanDriver
        fi
    elif [[ "$IPSEC_PACKAGE" == "libreswan" ]]; then
        iniset_multiline $Q_VPN_CONF_FILE vpnagent vpn_device_driver neutron_xyz.services.vpn.device_drivers.libreswan_ipsec.LibreSwanDriver
    else
        iniset_multiline $conf_file vpnagent vpn_device_driver $NEUTRON_VPNAAS_DEVICE_DRIVER
    fi
}

function neutron_xyz_start {
    local cfg_file
    ...
    run_process neutron-xyz "$AGENT_VPN_BINARY $opts"
}

function neutron_xyz_stop {
    ...
    stop_process neutron-xyz
}

function neutron_xyz_generate_config_files {
...
echo $NEUTRON_VPNAAS_DIR
if [[ "$1" == "stack" && "$2" == "install" ]]; then
    echo_summary "Installing neutron-xyz"
    neutron_xyz_install

elif [[ "$1" == "stack" && "$2" == "post-config" ]]; then
    neutron_xyz_generate_config_files
    neutron_xyz_configure_common
    if is_service_enabled q-svc; then
        echo_summary "Configuring neutron-vpnaas on controller"
        neutron_xyz_configure_db
    fi
    if is_service_enabled q-l3; then
        echo_summary "Configuring neutron-vpnaas agent"
        neutron_xyz_configure_agent
    fi
